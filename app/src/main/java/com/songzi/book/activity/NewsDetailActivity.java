...
import com.songzi.book.fragment.NewsDetailFragment;
import com.songzi.book.fragment.NewsDetailFragment.OnContentLoadListener;
...
public class NewsDetailActivity extends BaseActivity implements OnContentLoadListener {
	...
	public boolean onOptionsItemSelected(MenuItem item) {
		...
		case R.id.menu_item_fav_action_bar:
			...
			if (isInFavorite) {
				BookApplication.getNewsFavoriteDataSource().deleteFromFavorite(String.valueOf(mNewsId));
				...
				if (!TextUtils.isEmpty(title) && !TextUtils.isEmpty(share_url)) {
					BookApplication.getNewsFavoriteDataSource().add2Favorite(
	...
	private class FavoriteStatusGetTask extends MyAsyncTask<Void, Void, Boolean> {
		...
		protected Boolean doInBackground(Void... params) {
			...
			return BookApplication.getNewsFavoriteDataSource().isInFavorite(String.valueOf(mNewsId));
		}

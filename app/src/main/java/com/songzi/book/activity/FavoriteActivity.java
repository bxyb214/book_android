...
public class FavoriteActivity extends BaseActivity {
		...
		public boolean onActionItemClicked(ActionMode mode, MenuItem item) {
			...
			for (int i = 0; i < selected.size(); i++) {
				if (selected.valueAt(i)) {
					...
					BookApplication.getNewsFavoriteDataSource().deleteFromFavorite(String.valueOf(selectedItem.id));
				}
			}
	...
	public boolean onOptionsItemSelected(MenuItem item) {
		...
		case R.id.action_clear:
			BookApplication.getNewsFavoriteDataSource().deleteFromFavorite();
			new LoadCacheNewsTask().executeOnExecutor(MyAsyncTask.THREAD_POOL_EXECUTOR);
			return true;
		}
	...
	private class LoadCacheNewsTask extends MyAsyncTask<String, Void, ArrayList<NewsEntity>> {
		...
		protected ArrayList<NewsEntity> doInBackground(String... params) {
			...
			return BookApplication.getNewsFavoriteDataSource().getFavoriteList();
		}
